 ...
 * @date 2021-08-10
 */
public final class NetKafkaBytesChannel extends NetKafkaChannel<byte[], byte[]> {
	 ...
	 */
	private NetKafkaIOBytes toRemoteIO;
	 ...
	 */
	public NetKafkaBytesChannel(NetServiceHandler service, String channelID, NetKafkaConfigConsumer consumerConfig, NetKafkaConfigProducer producerConfig) throws IllegalArgumentException {
	...
	public final NetIO<?, ?> getToRemoteIO() {
		...
		// Gets the sender.
		NetKafkaSender<byte[], byte[]> sender = this.getSender();
		if (sender == null) return null;
		// Create a to remote I/O object.
		synchronized (lock) {
			if (toRemoteIO != null) return toRemoteIO;
			toRemoteIO = new NetKafkaIOBytes(this, null, sender);
		}
		return toRemoteIO;
	}

	@Override
	protected final NetIO<?, ?> getFromRemoteIO(KafkaConsumer<byte[], byte[]> consumer, ConsumerRecord<byte[], byte[]> record) {
		return new NetKafkaIOBytes(this, new NetKafkaReader<>(consumer, record), this.getSender());
	}
