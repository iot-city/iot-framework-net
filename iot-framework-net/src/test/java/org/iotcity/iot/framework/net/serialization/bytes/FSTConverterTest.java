 ...
 * @date 2021-09-17
 */
public class FSTConverterTest extends TestCase {
	...
	public void testSerializable() {
		...
		if (FSTBytes.FST_CLASS != null) {
			...
			try {
				// Test one data.
				byte[] tbs = fstBytes.serialize(list.get(0));
				JavaHelper.log("Bytes size: " + tbs.length);

				JavaHelper.log("RUN IN ONE THREAD...");
				// Run test.
				runFSTData(list, bytes, fstBytes);
				...
				for (int i = 0; i < THREADS; i++) {
					...
					handler.run(new PriorityRunnable() {
						...
						public void run() {
							try {
								runFSTData(listCopy, bytesCopy, fstBytes);
							} catch (Exception e) {
								e.printStackTrace();
							}
						}
		...
		JavaHelper.log("----------------------------- TEST FST COMPLETE -----------------------------");

	}

	private void runFSTData(List<Serializable[]> list, List<byte[]> bytes, FSTBytes fstBytes) throws Exception {

		long startTime1 = System.currentTimeMillis();
		for (Serializable[] sers : list) {
			bytes.add(fstBytes.serialize(sers));
		}
